package Aula_17_Algoritmo_De_Busca;
...
public class Principal {
	...
	private static void buscaBinaria(int n, int[] vetorOrdenado) {
		...
		while (inicio <= fim) {
			...
			if (posicaoMedia >= vetorOrdenado.length || posicaoMedia < 0) break;
			...
			if (n < vetorOrdenado[posicaoMedia]) fim = posicaoMedia - 1;
			else inicio = posicaoMedia + 1;
		}
		
		if (encontrou) System.out.println("Encontrei o número " + n + " na posição " + posicaoMedia);
	...
	private static void buscaLinear(int n, int[] vetorDesodernado) {
		...
		if (encontrou) System.out.println("Encontrei o número " + n + " na posição " + posicao);
		else System.out.println("Não encontrei este número no vetor");
	}
}
